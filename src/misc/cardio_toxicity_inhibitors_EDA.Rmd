...
## Kinases for CORAL visulaization
```{r}
cardiac_kinases = cardio_toxicity_data_full_long %>% 
	filter(QT_interval_prolongation == 2 | LV_dysfuntion == 2 | HF == 2 | Arrhythmia == 2 | Ischemia_or_MI == 2 | Hypertension == 2 | Others == 2) %>%
	filter(relative_intensity < 0.5)  %>% 
	select(kinase)  %>% 
	unique()

cardiac_kinases_entrez = bitr(cardiac_kinases$kinase, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Hs.eg.db::org.Hs.eg.db)

library(clipr)

cardiac_kinases_entrez %>%
	ungroup()  %>% 
	select(ENTREZID)  %>% 
	mutate(group = "Hit_Causes_Cardiac_Event_>10%") %>%
	select(ENTREZID, group)  %>% 
	as_tibble()  %>%
	write_clip()
```
