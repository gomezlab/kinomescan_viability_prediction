...
all_combos_extra = all_combos_releveled %>%
	...
	mutate(tested_origin = case_when(
		tested == 1 & origin == "Kinobeads" ~ "Tested Kinobeads",
		tested == 1 & origin == "KINOMEscan" ~ "Tested KINOMEscan",
		tested == 0 & origin == "Kinobeads" ~ "Untested_Kinobeads",
		tested == 0 & origin == "KINOMEscan" ~ "Untested_KINOMEscan",
		T ~ origin
	))
...
```{r}
ggplot(all_combos_extra, aes(y=drug_id_num,x=depmap_id_num, fill = as.factor(tested_origin), width=1, height=1)) +
	geom_tile() +
	scale_fill_manual(values = c("black", wes_palettes$GrandBudapest1[2], "white", "white")) +
	theme_void() +
  ...
  annotate("rect", xmin=0, xmax=1379, ymin=0, ymax=212, alpha=0.15, fill= wes_palettes$GrandBudapest2[3]) +
	annotate("text",x=233, y=193, colour = wes_palettes$GrandBudapest2[3],
	...
	annotate("text",x=255, y=985, colour =wes_palettes$GrandBudapest2[4],
					 label="KINOMEscan Compound Space") +
	labs(fill = "Origin & Test Status")
