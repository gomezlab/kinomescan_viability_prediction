...
```{r}
full_model_data_set_5000feat <- read_rds(here('results/PRISM_LINCS_klaeger_models_ic50/PRISM_LINCS_klaeger_data_for_ml_5000feat_ic50.rds.gz'))
...
```{r}
model_features = data.frame(temp = names(full_model_data_set_5000feat)) %>%
...
write_rds(model_features,here('results/PRISM_LINCS_klaeger_models_ic50/expression_feature_interactors.rds'))
```

```{r}
activation_genes = data.frame(temp = names(full_model_data_set_5000feat)) %>%
	separate(temp, into = c("type",'hgnc'), sep = "_") %>%
	separate_rows(hgnc,sep = ";") %>%
	filter(!is.na(hgnc), hgnc != "viability", hgnc != "id", hgnc != "conc") %>%
	filter(type == "act") %>%
