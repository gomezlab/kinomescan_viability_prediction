...
ic50_correlations = read_csv(here('results/PRISM_LINCS_klaeger_models/PRISM_LINCS_klaeger_all_multiomic_data_feature_correlations.csv')) %>% 
	mutate(feature_type = 
...
PRISM_klaeger_samples = ic50_data %>%
	...
	mutate(gene = fct_relevel(gene, "RPS6KA5", "MAP2K2", "CYC1"))
...
PRISM_klaeger_samples = ic50_data %>%
	...
	mutate(gene = fct_relevel(gene, "TMEM14C", "KCTD7", "ERCC2"))
...
activation_hex = ggplot(PRISM_klaeger_samples, aes(x=expression,y=ic50)) +
	# geom_hex(fill = log10(..count..)) +
	stat_binhex(aes(fill=log10(..count..))) +
	geom_smooth(method = lm, color = 'red', lwd=1) +
