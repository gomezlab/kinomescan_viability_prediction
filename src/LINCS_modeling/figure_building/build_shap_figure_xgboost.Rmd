...
shap_inhibition %>%
	mutate(class = if_else(gene == "MEK2", "Light", class))  %>% 
	mutate(class = if_else(gene == "PIK4CB", "Light", class))  %>%
  slice(1:25) %>%
	arrange(mean_importance_value) %>% 
...
ggsave(here('figures/PRISM_LINCS_klaeger/inhibition_shap_figure_for_poster.png'), height = 2.8, width = 7.5, units = "in")
