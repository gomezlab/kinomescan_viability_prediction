...
data = r.data
feats = r.cors
feat_counts = [400,500,1000,1500,2000,3000,4000,5000]
...
for f in feat_counts:
    ...
    scores.to_csv('results/PRISM_LINCS_klaeger_models_ic50/all_datasets/regression/keras/scores/hps_keras_' + str(f) + '.csv', index=False)

    results = pd.DataFrame(rnd_search_cv.cv_results_)
    results.sort_values(by='rank_test_score').to_csv('results/PRISM_LINCS_klaeger_models_ic50/all_datasets/regression/keras/results/hps_keras_' + str(f) + '.csv')
