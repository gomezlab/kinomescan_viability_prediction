...
set.seed(2222)
	history <- model %>% fit(
		...
		write_rds(here('results/PRISM_LINCS_klaeger_models_ic50/activation_expression/regression/keras/final_keras_model.rds.gz'), compress = "gz")
...
full_predictions = data.frame(predicted_ic50 = predictions) %>% 
	bind_cols(ids) %>% 
