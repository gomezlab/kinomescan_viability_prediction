...
```{r}
# Load the data
predictions <- vroom(here("results/PRISM_LINCS_klaeger_models_ic50/final_model_predictions.csv"))
...
promiscuity_summary = full_predictions %>% 
	...
	summarise(mean_predicted_ic50 = mean(predicted_ic50)) 
	
promiscuity_summary %>%
	ggplot() +
	geom_point(aes(x = promiscuity_score, y = mean_predicted_ic50))
```
...
ggsave(here('figures/PRISM_LINCS_klaeger/keras_predictions_top_kinase_targets_per_disease.png'), width = 8, height = 6)
...
top_100_drugs_per_disease %>% 
	ggplot(aes(x = primary_disease, y = drug, fill = mean_predicted_ic50)) +
	geom_tile() +
	labs(x = " ", y = "Kinase", title = "Kinase Importance by Cancer Type", size = "Cell Lines Sensitive to the Given Kinase") +
		theme(
		legend.position = "bottom",
...
get_interesting_pred_plots <- function(this_cell_line_name = "BT474", these_prediction_results = BC_predictions) {
	...
	these_summary = these_predictions %>%
		group_by(drug) %>%
		summarise(predicted_ic50 = predicted_ic50, 
							null_prox = abs(mean(predicted_ic50 - 1))) %>%
	...
	high_eff_plot = ggplot(deadly_pred_set, aes(x=drug, y=predicted_ic50)) +
		geom_col() +
		geom_line() + 
		labs(x="Drug", 
				 y="Predicted AUC") +
    ...
    plot.background = element_rect(fill = "transparent",colour = NA)
		...
		null_eff_plot = ggplot(null_eff_set, aes(x=drug, y=predicted_ic50)) +
		geom_col() +
		geom_line() + 
		labs(x="Drug", 
				 y="Predicted AUC") +
...
full_plot = high_eff_plot + null_eff_plot + 
	plot_annotation(title = "Model Predictions", subtitle = paste0(line_name_extended))

return(full_plot)
}

dir.create(here('figures/PRISM_LINCS_klaeger/final_keras_model//BC_lines_predictions/'), showWarnings = F)
for (this_line in unique(BC_predictions$stripped_cell_line_name)) {
	this_plot = get_interesting_pred_plots(this_line)
	ggsave(here('figures/PRISM_LINCS_klaeger/final_keras_model//BC_lines_predictions/',paste0(this_line,".png")), this_plot, width=8,height=5)
}
```