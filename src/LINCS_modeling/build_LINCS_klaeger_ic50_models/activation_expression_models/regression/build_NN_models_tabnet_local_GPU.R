...
args = data.frame(feature_num = c(5000,10000))
...
for(i in 1:length(args$feature_num)) {
	...
	full_output_file = here('results/PRISM_LINCS_klaeger_models/activation_expression/regression/tabnet/results', 
													sprintf('%dfeat.rds.gz',args$feature_num)[i])
	
	pred_output_file = here('results/PRISM_LINCS_klaeger_models/activation_expression/regression/tabnet/predictions', 
													sprintf('%dfeat.rds.gz',args$feature_num)[i])
