...
full_dataset = read_rds(here('results/PRISM_LINCS_klaeger_models_ic50/PRISM_LINCS_klaeger_data_for_ml_5000feat_ic50.rds.gz'))
...
x_train = klaeger_only_data %>% 
		select(ic50_target, starts_with("act")) 

spec = feature_spec(x_train, ic50_target ~ .) %>%
...
write_rds(history, here('results/klaeger_only_keras_model_ic50.rds.gz'), compress = "gz")
```
...
spec = feature_spec(x_train, ic50 ~ .) %>%
	step_numeric_column(all_numeric()) %>%
	fit()
...
input <- layer_input_from_dataset(x_train %>% select(-ic50))
main_output = input %>% 
	layer_dense_features(dense_features(spec)) %>%
