...
```{r}
#read in data
data = read_rds(here('results/PRISM_LINCS_klaeger_models_ic50/PRISM_LINCS_klaeger_data_for_ml_10000feat_ic50.rds.gz'))
...
data = r.data
feats = r.cors
feat_counts = [6000,7000,8000,9000]
...
for f in feat_counts:
    ...
    scores.to_csv('results/PRISM_LINCS_klaeger_models_ic50/activation_expression/regression/keras/scores/hps_keras_' + str(f) + '.csv', index=False)

    results = pd.DataFrame(rnd_search_cv.cv_results_)
    results.sort_values(by='rank_test_score').to_csv('results/PRISM_LINCS_klaeger_models_ic50/activation_expression/regression/keras/results/hps_keras_' + str(f) + '.csv')
