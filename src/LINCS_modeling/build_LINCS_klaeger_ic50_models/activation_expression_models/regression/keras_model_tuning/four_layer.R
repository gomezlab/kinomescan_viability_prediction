...
full_output_file = here('results/PRISM_LINCS_klaeger_models_ic50/activation_expression/regression/keras/results/four_layer', 
...
for(j in 1:dim(grid)[1]) {
	...
	for (split_id in 1:5) {
		...
		x_train = training(split) %>% 
			select(ic50_target, starts_with(c("act_", "exp_")))
		x_val = testing(split) %>% 
			select(ic50_target, starts_with(c("act_", "exp_")))
