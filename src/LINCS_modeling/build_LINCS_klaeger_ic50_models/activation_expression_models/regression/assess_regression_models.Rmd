...
for (feature_number in c(100,200,300,400,500,1000,1500)) {
...
for (feature_number in c(100,200)) {
	this_model_results = read_rds(
...
all_model_metrics = all_rf_metrics %>% 
	...
	bind_rows(
		all_lr_metrics %>%
			mutate(model_type = 'xgboost', hyperparameter = "NA", value = 0)
	) %>% 
	#bind_rows(
	# 	all_NN_metrics %>%
...
best_act_exp_model = model_metrics_combined %>% 
	filter(data_type == "activation_expression") %>% 
	arrange(desc(mean)) %>% 
