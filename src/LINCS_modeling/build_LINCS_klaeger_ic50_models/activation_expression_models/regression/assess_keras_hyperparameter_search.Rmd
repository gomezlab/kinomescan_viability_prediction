...
#Keras Inhibition Expression Tuning Assessment
...
write_csv(all_keras_scores, here('results/PRISM_LINCS_klaeger_models_ic50/all_datasets/regression/keras/results/hyperparameter_search_metrics.csv'))
```
...
write_csv(best_keras_scores, here('results/PRISM_LINCS_klaeger_models_ic50/all_datasets/regression/keras/results/best_hyperparameter_search_metrics.csv'))
```
...
all_combined_all_scores = all_keras_scores %>% 
	mutate(data_type = "inhibition_expression") %>% 
	bind_rows(all_data_keras_scores %>% 
							mutate(data_type = "all_datasets"))
```
...
all_combined_best_scores %>%
	ggplot(aes(x = num_features, y = rsq, colour = data_type, shape = data_type)) +
	geom_point() +
#  geom_errorbar(aes(ymin=mean-std_err, ymax=mean+std_err), width=.2,
#                position=position_dodge(.9)) +
