...
dataset_2124 = read_csv(
	here('results/PRISM_LINCS_klaeger_models_ic50/activation_expression/regression/lasso_selected_data_kevin/lasso_select2124.csv'))

dataset_8269 = read_csv(
	here('results/PRISM_LINCS_klaeger_models_ic50/activation_expression/regression/lasso_selected_data_kevin/lasso_select8269.csv'))

dataset_9269 = read_csv(
	here('results/PRISM_LINCS_klaeger_models_ic50/activation_expression/regression/lasso_selected_data_kevin/lasso_select9269.csv'))

this_dataset = read_rds(here('results/PRISM_LINCS_klaeger_models_ic50/PRISM_LINCS_klaeger_data_for_ml_10000feat_ic50.rds.gz'))
folds = read_rds(here(here('results/cv_folds/PRISM_LINCS_klaeger_folds_10000_ic50.rds.gz')))
xgb_grid = read_rds(here('results/hyperparameter_grids/xgb_grid.rds'))

recipe_2124 = recipe(ic50 ~ ., this_dataset) %>%
...
recipe_8269 = recipe(ic50 ~ ., this_dataset) %>%
	update_role(-starts_with("act_"),
							-starts_with("exp_"),
							-starts_with("ic50"),
							new_role = "id variable") %>%
...
recipe_9269 = recipe(ic50 ~ ., this_dataset) %>%
	update_role(-starts_with("act_"),
							-starts_with("exp_"),
							-starts_with("ic50"),
							new_role = "id variable") %>%
