...
# 	job_name = sprintf('1i%d',feature_num)
# 
# 	command = sprintf('sbatch -N 1 -n 1 -p volta-gpu --constraint="rhel8" --job-name=%s --mem=30G --time=120:00:00 --qos gpu_access --gres=gpu:1 --mail-user=cujoisa@live.unc.edu   --wrap=\"echo \' module load r/4.1.0; module load gcc/9.1.0; module load cuda/11.4; Rscript src/LINCS_modeling/build_LINCS_klaeger_ic50_models/activation_expression_models/regression/keras_model_tuning/one_layer.R --feature_num %d \' | bash\"', job_name, feature_num)
# 
# 	# print(command)
# 	system(command)
# 
