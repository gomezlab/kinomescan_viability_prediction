...
for (feature_num in c(100,200,300,400,500,1000,1500,2000,3000,4000,5000)) {
	...
	command = sprintf('sbatch --job-name=%s --mem=20G -c 16 --time=96:00:00 --wrap "Rscript src/LINCS_modeling/build_LINCS_klaeger_ic50_models/activation_expression_models/regression/build_rf_models_ANOVA.R --feature_num %d"', job_name, feature_num)
...
# 
# 	command = sprintf('sbatch -N 1 -n 1 -p volta-gpu --job-name=%s --mem=20G --time=96:00:00 --qos gpu_access --gres=gpu:1 --mail-user=cujoisa@live.unc.edu   --wrap "Rscript src/LINCS_modeling/build_LINCS_klaeger_ic50_models/activation_expression_models/regression/build_xgboost_models_ANOVA_GPU.R --feature_num %d"', job_name, feature_num)
# 
# 	# print(command)
# 	system(command)
# 
