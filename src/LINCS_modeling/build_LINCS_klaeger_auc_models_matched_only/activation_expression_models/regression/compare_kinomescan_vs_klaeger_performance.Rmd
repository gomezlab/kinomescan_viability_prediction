...
best_rand_forest_model = model_metrics_combined %>% 
	filter(data_type == "inhibition_expression" &model_type == "random_forest") %>% 
	arrange(desc(mean)) %>% 
	filter(model_id == model_id[1] & num_features == num_features[1]) %>% 
	pivot_wider(names_from = hyperparameter, values_from = value)
...
keras_predictions = read_rds(
	here(
		"results/PRISM_LINCS_klaeger_models_auc/activation_expression/regression/keras/final_keras_cv_predictions.rds.gz"
		)
	)
```
...
if (file.exists(here('results/PRISM_LINCS_klaeger_models_auc/PRISM_LINCS_klaeger_tuned_random_forest_regression_model_auc.rds.gz'))) {
	rand_forest_results = read_rds(here('results/PRISM_LINCS_klaeger_models_auc/PRISM_LINCS_klaeger_tuned_random_forest_regression_model_auc.rds.gz'))
} else { 
...
rand_forest_results <- fit_resamples(
	...
	control = race_ctrl
		...
		write_rds(here('results/PRISM_LINCS_klaeger_models_auc/PRISM_LINCS_klaeger_tuned_random_forest_regression_model_auc.rds.gz'),
						compress = "gz")
toc()
}
...
if (file.exists(here('results/PRISM_LINCS_klaeger_models_auc/PRISM_LINCS_klaeger_tuned_lasso_regression_model_auc.rds.gz'))) {
	lasso_results = read_rds(here('results/PRISM_LINCS_klaeger_models_auc/PRISM_LINCS_klaeger_tuned_lasso_regression_model_auc.rds.gz'))
} else { 
...
lasso_results <- fit_resamples(
	...
	control = race_ctrl
		...
		write_rds(here('results/PRISM_LINCS_klaeger_models_auc/PRISM_LINCS_klaeger_tuned_lasso_regression_model_auc.rds.gz'),
						compress = "gz")
toc()
}
...
keras_comparison_metrics = data.frame(model_type = "4-layer NN",
																kinomescan_rsq = kinomescan_rsq,
																klaeger_rsq = klaeger_rsq,
																kinomescan_rmse = kinomescan_rmse,
																klaeger_rmse = klaeger_rmse,
...
final_comparison_metrics = bind_rows(all_comparisons, keras_comparison_metrics) %>% 
	write_csv(here('results/PRISM_LINCS_klaeger_models_auc/PRISM_LINCS_klaeger_data_origin_comparison_metrics_auc.csv'))
```
