...
```{r}
# Load the data
predictions <- read_csv(here("results/PRISM_LINCS_klaeger_models_auc/matched_only_models/final_model_predictions_xgboost.csv")) %>%
...
ggsave(here('figures/PRISM_LINCS_klaeger/matched_only_models/predictions_top_kinase_targets_per_disease_auc.png'), width = 8, height = 6)
...
dir.create(here('figures/PRISM_LINCS_klaeger/matched_only_models/final_xgboost_model/BC_lines_predictions'), showWarnings = F)
for (this_line in unique(BC_predictions$stripped_cell_line_name)) {
	this_plot = get_interesting_pred_plots(this_line)
	ggsave(here('figures/PRISM_LINCS_klaeger/matched_only_models/final_xgboost_model/BC_lines_predictions',paste0(this_line,".png")), this_plot, width=16,height=5)
}
```