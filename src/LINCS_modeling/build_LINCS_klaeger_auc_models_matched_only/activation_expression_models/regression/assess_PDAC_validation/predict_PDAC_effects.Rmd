...
final_xgboost_model = read_rds(here('results/PRISM_LINCS_klaeger_models_auc/matched_only_models/activation_expression/regression/xgboost/final_xgboost_model.rds.gz'))
```
...
PDAC_dataset_for_model = cell_line_drug_combos %>% 
	...
	select(drug, cell_line, any_of(names(this_dataset))) %>% 
...
rmse = rmse(validation_assessment$exp_auc, validation_assessment$predicted_auc)^2
```
...
ggsave(here('figures/PRISM_LINCS_klaeger/matched_only_models/validation/preliminary_PDAC_validation_assessment.png'), height = 10, width = 9.5, units = "cm")
```
