...
this_dataset = build_all_data_regression_viability_set(feature_correlations =  cors,
																											 num_features = 5001,
																											 all_data = data) %>% 
	write_rds(here('results/PRISM_LINCS_klaeger_all_multiomic_data_for_ml_5000feat_auc.rds'))

set.seed(2222)
folds = vfold_cv(this_dataset, v = 10) %>% 
	write_rds(here('results/PRISM_LINCS_klaeger_all_multiomic_data_folds_auc.rds'))
