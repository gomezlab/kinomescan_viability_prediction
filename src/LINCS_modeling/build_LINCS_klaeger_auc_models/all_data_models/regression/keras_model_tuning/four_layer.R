...
dir.create(here('results/PRISM_LINCS_klaeger_models_auc/all_datasets/regression/', 
								sprintf('keras/results/four_layer')), 
					 showWarnings = F, recursive = T)

full_output_file = here('results/PRISM_LINCS_klaeger_models_auc/all_datasets/regression/keras/results/four_layer', 
...
spec = feature_spec(this_dataset %>% 
											select(auc_target, starts_with(c("act", "exp_", "prot_", "cnv_", "dep_"))),
										auc_target ~ .) %>%
...
for(j in 1:dim(grid)[1]) {
	...
	for (split_id in 1:5) {
		...
		ids = testing(split) %>% 
			select(-starts_with(c("act", "exp_", "prot_", "cnv_", "dep_")), -auc_target)
