...
for f in feat_counts:
    ...
    rnd_search_cv.fit(X_train, y_train, epochs=500, batch_size=512,
                    ...
                    callbacks=[keras.callbacks.EarlyStopping(patience=20, min_delta = 0.00001, restore_best_weights=True)])
    ...
    scores.to_csv('results/PRISM_LINCS_klaeger_models_auc/all_datasets/regression/keras/scores/hps_keras_' + str(f) + '.csv', index=False)

    results = pd.DataFrame(rnd_search_cv.cv_results_)
    results.sort_values(by='rank_test_score').to_csv('results/PRISM_LINCS_klaeger_models_auc/all_datasets/regression/keras/results/hps_keras_' + str(f) + '.csv')
