...
for (feature_num in c(3000,4000,5000)) {
	...
	command = sprintf('sbatch -N 1 -n 1 -p volta-gpu --job-name=%s --mem=50G --time=90:00:00 --qos gpu_access --gres=gpu:1 --mail-user=cujoisa@live.unc.edu   --wrap "Rscript src/LINCS_modeling/build_LINCS_klaeger_auc_models/activation_expression_models/regression/keras_model_tuning/one_layer.R --feature_num %d"', job_name, feature_num)
