...
write_rds(all_PDAC_validation_results, here('results/validation_results/PDAC_validation/all_PDAC_validation_results_auc.rds'))
```
...
model_predictions_tidy = model_predictions %>%
	...
	write_csv(here('results/validation_results/PDAC_validation/all_PDAC_validation_predictions_auc.rds'))
```
...
```{r}
#make specific figure for grant 
validation_assessment_split %>% 
  left_join(all_individual_metrics %>% 
  						rename(PDAC_type = this_cell, origin = this_origin)) %>% 
	filter(origin == "Kinobeads" & PDAC_type != "Primary Tumour Line 2") %>% 
	ggplot(aes(x = predicted_auc, y = exp_auc)) +
	geom_point() +
	geom_smooth(method = "lm", color = wes_palettes$GrandBudapest1[2]) +
	geom_abline(intercept = 0, slope = 1, size = 0.5, colour = 'black', linetype = 3) +
  ...
  facet_wrap(~ PDAC_type)
ggsave(here('figures/PRISM_LINCS_klaeger/validation/PDAC_figure_grant.png'), height = 5, width = 10, units = "cm")
```
