...
```{r}
#read in data 
this_dataset = read_rds(here('results/PRISM_LINCS_klaeger_models_auc/PRISM_LINCS_klaeger_data_for_ml_auc.rds.gz'))
```
...
lr_final_spec = linear_reg(penalty = 1.084246e-10, mixture = 1) %>%
	set_engine("glmnet") %>% 
	set_mode("regression")
...
all_importance = vi(final_results %>% extract_fit_parsnip()) %>%
  ...
  mutate(feature_type = case_when(
	...
	write_csv(here('results/results/PRISM_LINCS_klaeger_models_auc/lasso_selected_features.csv'))
```

```{r}
#lasso feature selection EDA

all_importance = read_csv(here('results/PRISM_LINCS_klaeger_models_auc/lasso_selected_features.csv'))
