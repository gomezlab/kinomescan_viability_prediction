---
	title: "Assess kevin tabnet regression"
...
# 	here('results/PRISM_LINCS_klaeger_models_auc/PRISM_LINCS_klaeger_data_for_ml_5000feat_auc.rds.gz'))

#read in CV files
all_tabnet_preds = data.frame()
for (cv_number in (1:10)) {
	this_cv_set = read_csv(
		here('results/PRISM_LINCS_klaeger_models_auc/activation_expression/regression/cv_tabnet',
	...
	this_preds_set = read_csv(
		...
		rename("col_id" = ...1, "predicted_auc" = 2)
	
	this_full_preds = this_cv_set %>% 
		left_join(this_preds_set)
	
	all_tabnet_preds = bind_rows(all_tabnet_preds, this_full_preds)
}
...
ggsave(here('figures/PRISM_LINCS_klaeger/tuned_tabnet_PRISM_LINCS_klaeger_auc_regression_model.png'), width = 10.5, height = 10.5, units = "cm")
```
...
final_full_predictions = all_tabnet_preds %>% 
...
c = a+b
ggsave(here('figures/PRISM_LINCS_klaeger/tabnet_klaeger_kinomescan_comparison.png'), width = 12, height = 8)
```
