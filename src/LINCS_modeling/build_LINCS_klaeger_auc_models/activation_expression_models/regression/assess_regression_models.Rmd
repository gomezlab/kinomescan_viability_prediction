...
tic()
# feature_number = 500
# all_tabnet_metrics = data.frame()
# for (feature_number in c(500,1500,5000)) {
# 	this_model_results = read_rds(
# 		here('results/PRISM_LINCS_klaeger_models_auc/activation_expression/regression/tabnet/results',
...
# 	all_tabnet_metrics = bind_rows(all_tabnet_metrics, this_model_metrics)
# }


all_rf_metrics = data.frame()
for (feature_number in c(100,200,300,400,500,1000,1500,3000,4000,5000)) {
	this_model_results = read_rds(
...
for (feature_number in c(100,200,300,400,1000,1500,2000,3000)) {
	this_model_results = read_rds(
...
model_metrics_combined %>% 
  ...
  geom_errorbar(aes(ymin=mean-std_err, ymax=mean+std_err), width=.2,
	...
	labs(title = "KINOMEscan-Klaeger Single Dose models predicting AUC", x = "number of features", y = "R-squared") +
	facet_wrap(vars(model_type)) 

ggsave(here('figures/PRISM_LINCS_klaeger/all_regression_models_auc_metrics.png'), width = 10, height = 5)
```
