...
for (feature_number in c(100,200,300,400,500,1000,1500,2000)) {
	this_model_results = read_rds(
		here('results/PRISM_LINCS_klaeger_models_auc/activation_expression/regression/lr/results',
				 sprintf('%dfeat.rds.gz',feature_number)))
...
for (feature_number in c(100,200,300,400,500,1000,1500,2000,3000,4000,5000)) {
	this_model_results = read_rds(
		here('results/PRISM_LINCS_klaeger_models_auc/activation_expression/regression/NN/results',
				 sprintf('%dfeat.rds.gz',feature_number)))
...
keras_metrics = read_csv(here('results/PRISM_LINCS_klaeger_models_auc/activation_expression/regression/keras/results/best_hyperparameter_search_metrics.csv')) %>%
	mutate(
		layers = case_when(
		layers == "one_layer" ~ '1',
		layers == "two_layer" ~ '2', 
...
keras_all_data_metrics = read_csv(here('results/PRISM_LINCS_klaeger_models_auc/all_datasets/regression/keras/results/best_hyperparameter_search_metrics.csv')) %>%
	mutate(
		layers = case_when(
		layers == "one_layer" ~ '1',
		layers == "two_layer" ~ '2', 
...
all_model_all_data_metrics = read_csv(here('results/PRISM_LINCS_klaeger_models_auc/all_datasets/regression/all_model_metrics.csv'))
...
results <- fit_resamples(
	...
	write_rds(here('results/PRISM_LINCS_klaeger_models_auc/final_tuned_PRISM_LINCS_klaeger_auc_regression_model.rds'))
toc()
```
...
results <- fit_resamples(
	...
	write_rds(here('results/PRISM_LINCS_klaeger_models_auc/final_tuned_PRISM_LINCS_klaeger_auc_rand_forest_regression_model.rds'))
toc()
```

```{r}
results = read_rds(here('results/PRISM_LINCS_klaeger_models_auc/final_tuned_PRISM_LINCS_klaeger_auc_rand_forest_regression_model.rds'))
...
```{r}
results = read_rds(here('results/PRISM_LINCS_klaeger_models_auc/final_tuned_PRISM_LINCS_klaeger_auc_regression_model.rds'))
final_metrics = collect_metrics(results)
final_predictions = collect_predictions(results) %>% 
	rename("predicted_auc" = .pred)
