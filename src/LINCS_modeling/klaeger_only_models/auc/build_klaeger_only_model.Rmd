...
```{r}
#build 5000 feat individual datasets 
...
ggsave(here('figures/klaeger_only_models_auc/xgboost_klaeger_only_model_kinomescan_predictions.png'), height = 7, width = 10)
```
...
```{r}
#build 10000 feat individual datasets 
args = data.frame(feature_num = 10000)
full_annotated_dataset = full_dataset %>% 
	left_join(compound_match_list, by = 'broad_id') %>% 
	mutate(origin = if_else(
		is.na(klaeger_name),
...
klaeger_only_data %>% 
	write_rds(here('results/klaeger_only_models_auc/PRISM_klaeger_only_data_10000feat_auc.rds.gz'), compress = "gz")

set.seed(2222)
folds = vfold_cv(klaeger_only_data, v = 5) %>% 
	write_rds(here('results/cv_folds/PRISM_klaeger_only_folds_10000_auc.rds.gz'), compress = "gz")
```