...
simple_recipe = recipe(binary_response ~ ., this_dataset) %>%
...
infgain_recipe = recipe(binary_response ~ ., this_dataset) %>%
	update_role(-starts_with("act_"),
							-starts_with("exp_"),
							-starts_with("binary_response"),
							new_role = "id variable") %>% 
	step_normalize(all_predictors()) %>% 
	step_select_infgain(all_predictors(), outcome = "binary_response", top_p = 10, threshold = 0.9)
...
vroom_write(cv_metrics_regression, here('results/PDX_all_models_classification_metrics_ANOVA.csv'))
write_rds(here('results/PDX_all_models_classification_results_ANOVA.rds'))