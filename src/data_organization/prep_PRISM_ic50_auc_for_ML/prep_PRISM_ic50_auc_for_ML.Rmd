...
write_csv(PRISM_ic50_for_ml, here('results/PRISM_LINCS_klaeger_models_ic50/PRISM_ic50_for_ml.csv'))

PRISM_ic50_auc_for_ml = response_curve_parameters %>% 
	select(depmap_id, ccle_name, ic50, auc, name, broad_id)

write_csv(PRISM_ic50_auc_for_ml, here('results/PRISM_LINCS_klaeger_models_auc/PRISM_auc_for_ml.csv'))
```

```{r}
#ic50 auc correlation
plot_data = PRISM_ic50_auc_for_ml %>% 
	filter(ic50 < 10) %>%
	filter(ic50 > 0.001) %>% 
