...
matched_drug_data %>%
	...
	geom_smooth() +
	labs(title = paste0('Correlation = ', round(cor(matched_drug_data$LINCS_relative_intensity, matched_drug_data$klaeger_relative_intensity), 3)),
			x = "KINOMEscan Inhibition State", y = "Kinobeads Inhibition State") +
	geom_abline(intercept = 0, slope = 1, size = 0.5, colour = 'red')

ggsave(here('figures/klaeger_LINCS_correlation.png'), width = 10, height = 10, units = "cm")
...
unmatched_LINCS_data = LINCS_long %>% 
	...
	filter(!(cid %in% matched_drug_data_summarised$cid & kinase %in% matched_drug_data_summarised$LINCS_kinase)) %>%
...
unmatched_klaeger_data = klaeger_tidy_1uM %>%
	...
	filter(!(cid %in% matched_drug_data_summarised$cid & gene_name %in% matched_drug_data_summarised$klaeger_kinase)) %>% 
...
write_csv(klaeger_LINCS_data_for_ml_wide, here('results/all_klaeger_LINCS_data_for_ml_wide.csv'))
